...
fn connect_to_neighbors(node_id: i32, nodes: &HashMap<i32, Node>) -> HashMap<i32, TcpStream> {
    ...
    for neighbor in &node.neighbors {
        ...
        if node_id < *neighbor {
            ...
            println!("Connection established with {}", neighbor);
        } else {
            // accept connections from neighbors with lower id
            (socket, _) = listener.accept().expect("Unable to accept connection");
            println!("Connection established with {}", neighbor);
        }
...
fn run_peleg(node_id: i32, nodes: &HashMap<i32, Node>) -> (i32, HashSet<i32>) {
    ...
    loop {
        ...
        for (id, stream) in &mut streams {
            ...
            let msg_type;
            if stop.is_none() && rounds_since_update <= 1 {
                msg_type = MessageType::Peleg(my_highest_uid, my_longest_distance, *id == parent);
            // <- highest_uid, longest_distance, is_child
            } else if stop == Some(*id) {
                done.insert(*id);
